...
- **逻辑层**：用 `ScrollTo*Offset` 推进真实偏移，但**节流**到较低频率（实现中是 `1/24s`）。
...
关键不是某个数字，而是**把“推进逻辑 offset”从每帧里拿出来**。
...
v3 支持横向与纵向两种滚动方向，并且提供一个非常实用的交互：按住 Shift 临时切换滚动方向。

... To be continued ...

### 7) 为什么滚动时要临时关闭 HitTest

渲染循环开始时把内容 `IsHitTestVisible` 设为 `false`，结束时恢复。这是一个典型的“把 UI 线程预算留给渲染”的策略：

- 高速滚动时，鼠标在大量元素上扫过会触发频繁的命中测试和状态变更（Hover、ToolTip、触发器）。
- 关闭命中测试能避免滚动过程中出现“莫名其妙卡一下”的尖峰。

当然，它也意味着滚动过程中无法点击内容。这是刻意的交互选择：滚动时更重要的是保持画面连续、避免误触。
